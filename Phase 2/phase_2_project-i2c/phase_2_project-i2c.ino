...
float pwmToVelocity(int pwmValue) {
  // Linear approximation: PWM 0-255 maps to 0-maxRPM
...
void calculateApproachVelocity(float currentDist) {
  ...
  if (deltaTime >= 50) {  // Update every 50ms for stability
    // Distance change in meters (convert from cm)
...
int calculateTTCSpeed(float distance, int targetSpeed) {
  ...
  if (closingVelocity <= 0.01) {
    return targetSpeed;  // Not approaching, no need to slow down
  }
  ...
  // Using kinematic equation: t = v / a
  ...
  // Add safety margin (1.5x stopping time/distance)
  ...
  if (ttc < requiredTTC || distance < requiredDistance) {
    ...
    // The closer we are to the limit, the more we slow down
    float ttcRatio = ttc / requiredTTC;
    float distRatio = distance / requiredDistance;
    
    // Use the more critical ratio (smaller value)
    float ratio = min(ttcRatio, distRatio);
    ratio = constrain(ratio, 0.0, 1.0);
    
    // Gradual speed reduction - never instant stop
